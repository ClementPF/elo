...
export default class RivalryRowsContainer extends Component {
    ...
    componentDidMount() {
        ...
        getRivalryForUserForRivalForTournament(
            ...
            this.props.tournamentName).then((response) => {
    ...
    render() {
        ...
        if(this.state.rivalry.rivalry_stats_id == null){
            return <View style={ { flex: 1, justifyContent: 'center', alignItems: 'center'} }>
                <Text>
                    NO RIVALRY BETWEEN {this.props.username} AND {this.props.rivalname} FOR {this.props.tournamentName}
                </Text>
            </View>
        }
        const rivalry = this.state.rivalry;
        return <RivalryCardRows
            name1= { 'Total Points' }
