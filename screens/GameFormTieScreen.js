...
/*
    ...
    fetchData(username, tournamentName){
        ...
        Promise.all([
            getStatsForUserForTournament(username, tournamentName).then((response) => {
...
_handleBarCodeRead = ({type, data}) => {
    if (!this.state.qrCodeRead) {
        ...
        if (jsonData.tournament.name != this.state.tournament.name) {
        ...
        }else if(this.state.score > jsonData.score ||
            ...
            this.dropdown.alertWithType('info', 'Info', 'The other player has to submit the game as he has a highest score, or you have the same score but he has been using the sharkualtor longer than you');
        }
    }
}
...
render() {
    ...
    let jsonObj = {'user': this.props.user,
...
export default connect(mapStateToProps, { fetchUser, fetchGamesForUser })(GameFormTieScreen);
