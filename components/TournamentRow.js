...
export default class TournamentRow extends Component {
  static propTypes = {
    ...
    tournamentIdName: PropTypes.string,
    sport: PropTypes.string,
    score: PropTypes.number,
    position: PropTypes.number
  };
  render = () => {
    const { tournament, tournamentIdName, sport, score, position } = this.props;

    return (
      <View style={stylesTournament.container}>
        <View style={{ flex: 1, flexDirection: 'column' }}>
          <Text style={stylesTournament.tournamentText}>{tournament}</Text>
          <Text style={stylesTournament.tournamentSubtitleText}>{'@' + tournamentIdName}</Text>
        </View>
